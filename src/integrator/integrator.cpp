...
SpectrumD Integrator::renderD_interior(const Scene &scene, int sensor_id) const {
    using namespace std::chrono;
    auto start_time = high_resolution_clock::now();
    SpectrumD result = __render<true>(scene, sensor_id);
    result = result - detach(result);
    cuda_eval(); cuda_sync();
    auto end_time = high_resolution_clock::now();
    if ( scene.m_opts.log_level ) {
        std::stringstream oss;
        oss << "renderD_interior in " << duration_cast<duration<double>>(end_time - start_time).count() << " seconds.";
        log(oss.str().c_str());
    }
    return result;
}

SpectrumD Integrator::renderD_primary(const Scene &scene, int sensor_id) const {
    using namespace std::chrono;
    auto start_time = high_resolution_clock::now();
    
    const RenderOption &opts = scene.m_opts;
    const int num_pixels = opts.width*opts.height;
    SpectrumD result = zero<SpectrumD>(num_pixels);
    ...
    if ( scene.m_opts.log_level ) {
        std::stringstream oss;
        oss << "renderD_primary in " << duration_cast<duration<double>>(end_time - start_time).count() << " seconds.";
        log(oss.str().c_str());
    }
    return result;
}

SpectrumD Integrator::renderD_secondary(const Scene &scene, int sensor_id) const {
    using namespace std::chrono;
    auto start_time = high_resolution_clock::now();
    ...
    if ( scene.m_opts.log_level ) {
        std::stringstream oss;
        oss << "renderD_secondary in " << duration_cast<duration<double>>(end_time - start_time).count() << " seconds.";
        log(oss.str().c_str());
    }
    return result;
}

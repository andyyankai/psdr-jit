...
template <bool ad>
DRJIT_INLINE Vector3f<ad> sphdir(const Float<ad> &theta, const Float<ad> &phi) {
...
template <bool ad>
DRJIT_INLINE Vector3f<ad> bilinear(const Vector3f<ad> &p0, const Vector3f<ad> &e1, const Vector3f<ad> &e2, const Vector2f<ad> &st) {
    return fmadd(e1, st.x(), fmadd(e2, st.y(), p0));
}


template <bool ad>
DRJIT_INLINE Vector2f<ad> bilinear2(const Vector2f<ad> &p0, const Vector2f<ad> &e1, const Vector2f<ad> &e2, const Vector2f<ad> &st) {
    return fmadd(e1, st.x(), fmadd(e2, st.y(), p0));
}


template <bool ad>
DRJIT_INLINE Float<ad> rgb2luminance(const Vector3f<ad> &rgb) {
    return rgb.x()*.2126f + rgb.y()*.7152f + rgb.z()*.0722f;
}


template <bool ad>
DRJIT_INLINE auto ray_intersect_triangle(const Vector3f<ad> &p0, const Vector3f<ad> &e1, const Vector3f<ad> &e2, const Ray<ad> &ray) {
...
DRJIT_INLINE auto ray_intersect_box(const Ray<ad> &ray, const Vector3f<ad> &lower, const Vector3f<ad> &upper) {
    ...
    Vector3f<ad> t1p = drjit::min(t1, t2),
                 t2p = drjit::max(t1, t2);
...
DRJIT_INLINE Vector3fC squareToEdgeRayDirection_NB(const Vector2fC &sample, const Vector3fC &n0, const Vector3fC &n1, FloatC &pdf) {
...
DRJIT_INLINE std::pair<Vector3fC, Vector3fC> coordinate_system2(const Vector3fC &n) {
    FloatC sign = drjit::sign(n.z()),
          a    = -rcp(sign + n.z()),
          b    = n.x() * n.y() * a;
...
DRJIT_INLINE Vector3fC squareToEdgeRayDirection_B(const Vector2fC &sample, const Vector3fC &n0, const Vector3fC &e0, const Vector3fC &e1) {
    ...
    z = drjit::minimum(z, FloatC(1.0f - EdgeEpsilon));

    FloatC r = sqrt(drjit::maximum(FloatC(0.0f), FloatC(1.0f - z*z)));
...
template <bool ad>
DRJIT_INLINE Float<ad> mis_weight(const Float<ad> &pdf1, const Float<ad> &pdf2) {
    Float<ad> w1 = sqr(pdf1), w2 = sqr(pdf2);
    return w1/(w1 + w2);
}

template <bool ad>
DRJIT_INLINE Float<ad> mis_weight2(const Float<ad> &pdf1, const Float<ad> &pdf2) {
